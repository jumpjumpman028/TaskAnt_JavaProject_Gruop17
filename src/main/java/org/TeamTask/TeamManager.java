...
public class TeamManager {
    ...
    public void fetchTeamsFromDatabase() {
        String selectSQL = "SELECT team_id, team_name, team_code, team_member FROM teams";
...
//                System.out.println(teamMembers.contains(UserInfo.userID));
    ...
    public boolean addTeam(Team newTeam) {
        String insertSQL = "INSERT INTO teams (team_name, team_code, team_member, member_count) VALUES (?, ?, ?, ?)";
    ...
    public boolean updateTeamsToDatabase() {
        String updateSQL = "UPDATE teams SET team_name = ?, team_code = ?, team_member = ?, member_count = ? WHERE team_id = ?";
    ...
    public void joinTeam(String teamcode) {
        // 查詢團隊的 SQL
        String selectSQL = "SELECT team_id, team_name, team_code, team_member FROM teams WHERE team_code = ?";
            ...
            try (ResultSet resultSet = selectStatement.executeQuery()) {
                if (resultSet.next()) {
                    ...
                    if (rowsAffected > 0) {
                        ...
                        // 更新本地的 teamList
                        Team updatedTeam = new Team(teamId, teamName, teamCode, teamMembers);
